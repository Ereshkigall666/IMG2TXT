...
KRAKEN_SEG_MODELS_DB: Final[str] = os.path.join(
    KRAKEN_DATA_DIR, "segmentation_models.json")
...
# KRAKEN_MODELS: Final[dict[dict]] = {"eng": {"kraken_key": "10.5281/zenodo.2577813", "doi": "2577813", "name": "en_best.mlmodel"}"fra-lectaurep": {"kraken_key": "10.5281/zenodo.6542744","doi": "6542744", "name": "lectaurep_base.mlmodel"}, "fra": {"kraken_key": "10.5281/zenodo.6657809", "doi": "6657809", "name": "HTR-United-Manu_McFrench.mlmodel"}}
with open(KRAKEN_OCR_MODELS_DB, "r") as f:
    KRAKEN_MODELS: Final[dict[dict[str, str]]] = json.loads(f.read())
with open(KRAKEN_SEG_MODELS_DB, "r") as f:
    KRAKEN_SEG_MODELS: Final[dict[dict[str, str]]] = json.loads(f.read())
